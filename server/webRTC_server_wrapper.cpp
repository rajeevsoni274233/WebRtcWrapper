/**
 * (C) Copyright Elektrobit Automotive GmbH
 * All rights reserved
 * @ Author: Rajeev Soni
 * @ Create Time: 2022-10-03 18:35:15
 * @ Modified by: Your name
 * @ Modified time: 2023-06-14 15:16:43
 * @ Description:
 */
#include "include/webRTC_server_wrapper.h"

webRTC_server_wrapper::webRTC_server_wrapper()
{
}

webRTC_server_wrapper::~webRTC_server_wrapper()
{
}

void webRTC_server_wrapper::init()
{
}

void webRTC_server_wrapper::connectToPeer()
{
}

void webRTC_server_wrapper::disconnectFromPeer()
{
}

void webRTC_server_wrapper::sendData(void *data)
{
}
